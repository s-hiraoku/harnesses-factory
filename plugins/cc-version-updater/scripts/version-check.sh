...
show_post_upgrade_summary() {
  ...
  local prev_version=$(jq -r '.previousVersion // "unknown"' "$CHANGELOG_SUMMARY_FILE")
  local new_version=$(jq -r '.latestVersion // "unknown"' "$CHANGELOG_SUMMARY_FILE")

  # Use temp file to avoid shell parsing issues with special characters
  local tmp_file=$(mktemp)
  trap "rm -f '$tmp_file'" EXIT

  # Extract summary and convert escape sequences safely
  jq -r '.summary // "Summary not available"' "$CHANGELOG_SUMMARY_FILE" > "$tmp_file"

  # Convert \033 to actual escape characters
  # Read entire file content and use printf %b for conversion
  # Using a subshell with bash to ensure consistent behavior
  local summary=$(bash -c 'printf "%b" "$(cat "$1")"' _ "$tmp_file")
  rm -f "$tmp_file"
...
$summary"
  ...
  local context_msg="[cc-version-updater] Claude Code has been upgraded from v${prev_version} to v${new_version}. The summary above has been displayed. If the user asks follow-up questions, refer to the changelog-interpreter skill for guidance."
